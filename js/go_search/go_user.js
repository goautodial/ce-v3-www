...
/*$(function (){
    ...
    $("#userform").validate({
            ...
            submitHandler: function(form) {
                     $.post(
                            "https://"+window.location.host+"/index.php/go_user/autogenuser/"+account,
                            $("#userform").serialize(),
                            function (data){
...
$(function (){
    ...
    $('#accountNum').change(function (){ 
            ...
            $.post(
                   "https://"+window.location.host+"/index.php/go_user/getaccountinfo/"+account,
                   $("#userform").serialize(),
                   function (data){
                        var infos = $(data);
...
$(document).ready(function (){
    ...
    $('div.ungroup-toggle').click(function (){
        ...
        $("#"+$group).slideToggle("fast",function(){
              $(this).parent().toggleClass("closed");
              /*$.get(
                     protocol+"//"+host+"/index.php/go_user/collectuserinfo/"+$("#vicidial_user_id-"+$userid).val(),
              ...
              /*$.ajax({
                      type: "GET",
                      url: protocol+"//"+host+"/index.php/go_user/collectuserinfo/"+$("#vicidial_user_id-"+$userid).val(),
                      async: false,
                      datatype : 'jsonp',
                      success: 
              });*/
...
$(document).ready(function (){
    ...
    $('div[class|="draggable"]').each(function (index){
        var $id = $(this).attr("id");
        $("#"+$id).click(function(){
            $('.inside').load(protocol+"//"+host+"/index.php/go_user/modifyuser/",{userid:$id});
        });
    });
        ...
        if($action.toLowerCase() == 'update'){
            $("#form-"+$group).validate({
                   submitHandler: function(form){
                       if($(this).valid){
                             $.post(
                                    protocol+"//"+host+"/index.php/go_user/updateuser/"+$('#vicidial_user_id-'+$group).val()+"/"+$("#users_id-"+$group).val(),
            ...
            $.post(
                  protocol+"//"+host+"/index.php/go_user/deleteuser/"+$('#vicidial_user_id-'+$group).val(),
                  function (data){
            ...
            $('.inside').load(protocol+"//"+host+"/index.php/go_user/advancemodifyuser/",{userid:$group});
            return false;
        }
                                     ...
                                     function(){
                                        if($(this).hasClass('inactive') !== false){
                                            try{
                                                ...
                                                $.post(
                                                       protocol+'//'+host+'/index.php/go_user/deleteitem/',
       ...
       if($elementid == "save-group"){
          ...
          $("#user-groups").children().each(function($index,$value){            
             ...
             if($(this).hasClass('inactive') === false){ 
                  ...
                  if($(this).hasClass('new-user-groups') === true){ 
                         ...
                         if($formObj.is('form') === true){
                             ...
                             $("#"+$formid).validate({
                                  submitHandler:function (form){
                                      ...
                                      $.post(
                                               protocol+'//'+host+"/index.php/go_user/saveusergroup/"+$currentVuserid+"/"+$newgroupid,
                                               $finalarray,
                                               function (data){
                                                    alert(data);
                     ...
                     if($(this).is('div') === true){ // check if it is div if so proceed in saving 
                        ...
                        $.each($.makeArray($cloned),function(){
                         ...
                         $.post(
                             protocol+'//'+host+"/index.php/go_user/saveusergroup/"+$currentVuserid+"/"+$groupid,
               ...
               $.post(
                   protocol+'//'+host+"/index.php/go_user/createusergroup",
               ...
               if($(this).is('div')){
                   if($(this).hasClass('inactive') === false){
                       ...
                       if(confirm("Do you want to Add this user to the active group?")){ 
                            $.post(
                                  protocol+'//'+host+'/index.php/go_user/updategousers/'+$currentid.substring($currentid.indexOf("-user-")+6),
        ...
        if($(this).text() === "Save User"){
              ...
              $("form[id^='adv-userinfo-']").validate({
                      submitHandler: function(){
                             $.post(
                                    protocol+"//"+host+"/index.php/go_user/updateuser/"+$elementid.substring($elementid.indexOf('action-')+7)+"/"+$advanceformuserid,
                                    $finaldata,
                                    function (data){
                                        alert(data);
                                        // reset to null again
