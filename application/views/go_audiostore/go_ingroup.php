...
<script language="javascript">
	...
	function postval(groupid) {
		...
		var items = $('#showlistview').serialize();
			$.post("http://192.168.100.112/index.php/go_ingroup/editview", { items: items, action: "editlist" },
				 function(data){
	...
	function editpost(groupID) {
			...
			var itemsumit = $('#edit_go_listfrm').serialize();
				$.post("http://192.168.100.112/index.php/go_ingroup/editsubmit", { itemsumit: itemsumit, action: "editlistfinal" },
				function(data){
  ...
  function launch_chooser(fieldname,stage,vposition,defvalue) {
		...
		$.post("http://192.168.100.112/index.php/go_ingroup/chooser", { action: "sounds_list",user: "<?=$gouser?>", format: "selectframe", stage: stage, comments: fieldname, defaultvalue: defvalue  },
		function(data){

			if(fieldname=="after_hours_message_filename"){
 				document.getElementById("divafter_hours_message_filename").innerHTML=data;
 			} 
		
			if(fieldname=="welcome_message_filename"){
 				document.getElementById("divwelcome_message_filename").innerHTML=data;
 			} 
 				
			if(fieldname=="onhold_prompt_filename"){
 				document.getElementById("divonhold_prompt_filename").innerHTML=data;
 			}
 				
			if(fieldname=="eht_minimum_prompt_filename"){
	 			document.getElementById("diveht_minimum_prompt_filename").innerHTML=data;
	 		}
	 				
			if(fieldname=="wait_time_option_press_filename"){
	 			document.getElementById("divwait_time_option_press_filename").innerHTML=data;
			}
	 				
	 		if(fieldname=="wait_time_option_callback_filename"){
	 			document.getElementById("divwait_time_option_callback_filename").innerHTML=data;
	 		}
			
			if(fieldname=="hold_time_option_press_filename"){
	 			document.getElementById("divhold_time_option_press_filename").innerHTML=data;
			}
	 				
	 		if(fieldname=="hold_time_option_callback_filename"){
	 			document.getElementById("divhold_time_option_callback_filename").innerHTML=data;
			}
	
			if(fieldname=="agent_alert_exten"){
	 			document.getElementById("divagent_alert_exten").innerHTML=data;
			}                				
	...
	function launch_vm_chooser(fieldname,stage,vposition,defvalue) {
	    ...
	    $.post("http://192.168.100.112/index.php/go_ingroup/chooser", { action: "vm_list",user: "<?=$gouser?>", format: "selectframe", stage: stage, comments: fieldname, defaultvalue: defvalue  },
		function(data){
	...
	function launch_moh_chooser(fieldname,stage,vposition,defvalue) {
			...
			$.post("http://192.168.100.112/index.php/go_ingroup/chooser", { action: "moh_list",user: "<?=$gouser?>", format: "selectframe", stage: stage, comments: fieldname, defaultvalue: defvalue  },
			function(data){
	...
	function checkdatas(groupID) {
		...
		$.post("http://192.168.100.112/index.php/go_ingroup/checkagentrank", { itemrank: itemdatas, action: "getcheckagentrank", idgroup: groupID  },
			function(data){
				//alert(data);			
				/*var datas = data.split(":");
              var i=0;
...
/*	            top:-200px;*/
			...
			.tredit td{ background:#EFFBEF; font-family: Verdana; color:#000;  border-bottom: 1px dashed rgb(208,208,208); }   
...
<div id='outbody' class="wrap">
    ...
    <h2 style="font-family: Verdana;">In-groups</h2>
    <br><!-- spacer -->
            ...
            <div class="postbox-container" style="width:95%;">
                <div class="meta-box-sortables ui-sortables">
                    ...
                    <div class="postbox">
                        ...
                        <h3 class="hndle" >
                            <span style="font-family: Verdana;">In-group Listings</span>
...
<!--				<table cellspacing="1" cellpadding="1" border="0" style="margin-left:auto; margin-right:auto; width:95%; border:#D0D0D0 solid 1px; -moz-border-radius:5px; -khtml-border-radius:5px; -webkit-border-radius:5px; border-radius:5px; ">-->
				...
				<table cellspacing="1" cellpadding="1" border="0" style="margin-left:auto; margin-right:auto; width:100%;">
					<tr align="center" style="font-family: Verdana;">
						<td><b>IN-GROUP</b> <img src="http://192.168.100.112/img/arrow_down.png"></td>
						<td colspan="2"><b>DESCRIPTIONS</b> <img src="http://192.168.100.112/img/arrow_down.png"></td>
						<td><b>PRIORITY</b> <img src="http://192.168.100.112/img/arrow_down.png"></td>
						<td><b>STATUS</b> <img src="http://192.168.100.112/img/arrow_down.png"></td>
						<td><b>TIME</b> <img src="http://192.168.100.112/img/arrow_down.png"></td>
						<!--<td><b>COLOR</b></td>-->
						<td><b>ACTION</b> <img src="http://192.168.100.112/img/arrow_down.png">  </td>
						<td><input id="selectall" value="selectall" type="checkbox"></td>
					</tr>
					<?php
					   foreach($ingrouplists as $ingroupInfo){
					?>   		         	
							 <tr align="left" class="tr<?php echo alternator('1', '2') ?>">
								 <td align="" bgcolor="<?=$rowColor?>">
									<? echo $ingroupInfo->group_id; ?></td>
								 <td colspan="2">
								 <?
								 ...
								 <td align="center"><? echo $ingroupInfo->call_time_id; ?></td>
								 
								<?php
										
										/*$strex = explode('#', $ingroupInfo->group_color);
										$cnt = count($strex);
										if($cnt > 1) {
											echo "<td style=\"background: #$ingroupInfo->group_color ;\">1";	
										} else {
											echo "<td style=\"background: $ingroupInfo->group_color ;\">2";
										}*/
								?>								 
								 <!--<font size="1">--> 
								 <? //$ingroupInfo->group_color;?> <!--</font></td>-->
  								 <td align="center"  style="padding: 3px 0;">
  								 
									<img src="<?=$base?>img/edit.png" onclick="postval('<? echo $ingroupInfo->group_id; ?>');" style="cursor:pointer;width:14px;" title="MODIFY <?=ucwords(strtolower($ingroupInfo->group_name))?>" />
				   ...
				   <div align="left" class="title-header" style="font-family: Verdana;">Create new In-Group</div>
						...
						<table class="tableedit" width="100%" >
						  <tr>
								<td>Group ID: </td>
								<td>
								<input type="text" name="group_id" id="group_id" size="30"><br>
		                ...
		                <tr>
								<td>Group Name: </td>
								<td>
								<input type="text" name="group_name" id="group_name" size="30"><br>
									<font color="red" size="1">
		                ...
		                <tr>
		                		<td>Group Color: </td>
		                		<td>
		                			<input class="color" type="text" name="group_color" id="group_color" size="7" maxlength="7" value="66ff00">
		                		</td>
		                </tr>
		                <tr>
		                		<td>Active: </td>
		                ...
		                <tr>
		                		<td>Web Form: </td>
		                		<td>
									<input type="text" name="web_form_address" size="50" maxlength="1055">
								</td>
						  </tr>
		                <tr>
		                		<td>Voicemail: </td>
		                		<td>
		                			<input type="text" name="voicemail_ext" id="voicemail_ext" size=10 maxlength=10>
		                		</td>
		                </tr>
		                <tr>
		                		<td>Next Agent Call: </td>
		                		<td>
