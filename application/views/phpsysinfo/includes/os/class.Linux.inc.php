...
class sysinfo {
	...
	function sysinfo() {
		...
		if ( $distro_info != 'ERROR') {
			...
			$goautodialrelease = '/etc/goautodial-release';
			if ( file_exists($goautodialrelease) )
				{
					$file = fopen($goautodialrelease, "r") or exit("Unable to open file!");
  					$distrodesc=fgets($file);
  					fclose($file);
  					$distro='GoAutoDial';
  					$this->icon = isset($list[$distro]["Image"]) ? $list[$distro]["Image"] : $this->icon;
					$this->distro = $distrodesc;
				}
				else
				{
