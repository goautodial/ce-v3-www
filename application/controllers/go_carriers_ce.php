...
class Go_carriers_ce extends Controller {
	...
	function index()
	{
		...
		$data['bannertitle'] = $this->lang->line('go_carrier_banner');
	...
	function go_get_carrier()
	{
		...
		if($_POST['captcha'] !== $_SESSION['captcha']){
			echo $this->lang->line("go_error_invalid_captcha");
		} else {
		...
		if(!in_array($r,array(402,400,412,490,500))){
			...
			echo $this->lang->line("go_account_creation_success");
		} else {
			echo $this->lang->line("go_error_cant_create");
		}
            }
        }
...
// 			echo "<script>javascript: window.location = '$base'</script>";
        ...
        function copycarrier(){
            if(!empty($_POST)){
                ...
                if($isSippy > 0){
		       die(''.$this->lang->line('go_only_GAD_allowed').'');
                }
		...
		#		if (!$isExist) {
					...
					if(!empty($fields)){
						...
						echo $this->lang->line('go_success_copy_carrier');
						//}
					}else{
						echo $this->lang->line('go_failed_copy');
					}
		#		}else{
		...
		#		}
                ...
                echo $this->lang->line("go_empty_post_data");
            }
        }
		...
		if($havesippy->num_rows() > 0){
			die("".$this->lang->line('go_error_multi_gad_carrier')."");
		}
        }
