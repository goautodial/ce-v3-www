...
// 						var_dump("DELETE FROM vicidial_campaign_dnc WHERE phone_number='$dnc' AND campaign_id='".$string['campaign_id']."'");
	...
	function pagelinks($page,$rp,$total,$limit)
	{
		...
		$pageinfo = "<span style='float:right;padding-top:10px;'>{$this->lang->line('go_displaying')} {$pg['istart']} {$this->lang->line('go_to')} {$pg['iend']} {$this->lang->line('go_of')} {$pg['total']} {$this->lang->line('go_dnc_numbers')}</span>";
