...
class Go_phones extends Model {
	...
	function go_get_phone_list()
	{
		if (strlen($this->uri->segment(5)) > 0) {
			...
			$searchSQL = "WHERE (extension RLIKE '$search' OR server_ip RLIKE '$search' OR fullname RLIKE '$search')";
		}
		
		$query	= $this->db->query("SELECT count(*) AS cnt FROM phones $searchSQL;");
		$total	= $query->row()->cnt;
		...
		$return['pagelinks'] = $this->pagelinks($group,$page,$rp,$total,$limit,$search);
		
		$query = $this->db->query("SELECT extension,protocol,server_ip,dialplan_number,voicemail_id,status,active,fullname,messages,old_messages,user_group FROM phones $searchSQL ORDER BY extension LIMIT $start,$rp;");
	...
	function pagelinks($group_id,$page,$rp,$total,$limit)
	{
		...
		if ($pg['last'] > 1) {
			...
			if ($rp > 25) {
                ...
                $pageinfo = "{$this->lang->line('go_displaying')} {$pg['istart']} {$this->lang->line('go_to')} {$pg['iend']} {$this->lang->line('go_of')} {$pg['total']} {$this->lang->line('go_phones')}";
