...
class Go_moh extends Model {
	...
	function pagelinks($page,$rp,$total,$limit)
	{
		...
		if ($pg['last'] > 1) {
			...
			$pagelinks .= '<a title="'.$this->lang->line('go_to_1').'" style="vertical-align:baseline;padding: 0px 2px;" onclick="changePage('.$pg['prev'].')"><span><img src="'.base_url().'/img/prev.gif"></span></a>';
		...
		$sss = ($pg['total'] > 1) ? "(s)" : "";
		$pageinfo = "{$this->lang->line('go_displaying')} {$pg['istart']} {$this->lang->line('go_to')} {$pg['iend']} {$this->lang->line('go_of')} {$pg['total']} {$this->lang->line("go_moh_item")}{$sss}";
